...
(() => {
  ...
  function assess(items, sources, aqi, now = Date.now()) {
    ...
    const uncertain = items.some(item => !item.confirmed || item.stage === 'unconfirmed' || !item.severity.rank);
    const incomplete = !fresh('alerts') || !fresh('weather') || !fresh('air-quality') || uncertain;
