...
test('precache do SW lista os mesmos JS/CSS versionados do HTML', () => {
  ...
  assert.match(sw, /const CACHE = "pluvia-core-83"/);
  assert.match(html, /styles\.css\?v=core-83/);
  assert.match(html, /app\.js\?v=core-83/);
  assert.equal((sw.match(/pluvia-core-\d+/g) || []).every(token => token === 'pluvia-core-83'), true);
});
