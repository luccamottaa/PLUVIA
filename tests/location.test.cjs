...
const examples = nodes.get('citySearch').placeholder.replace(/^Ex\.: /,'').split(', ');
assert.equal(examples.length,3);
assert.equal(new Set(examples).size,3);
assert(examples.every(name=>run('CAPITALS.map(city=>city.name)').includes(name)));
...
assert.match(html, /<span id="cityName">Sua cidade<\/span>/);
