...
const catalog = fs.readFileSync(path.join(root,'dist/capitals.js'),'utf8');
...
(async()=>{
  ...
  assert.equal(vm.runInContext('activeCity.name',reloaded),'Brasília');
  assert.equal(vm.runInContext('favorites.has("1302603")',reloaded),true);
  console.log('PASS 27 capitals: catalog, regional INMET matching, coordinates, three timezones, search, favorites, per-city cache, saved selection and stale-request isolation.');
